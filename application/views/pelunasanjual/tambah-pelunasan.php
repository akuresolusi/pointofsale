...
<style type="text/css">
  ...
  #daftar-autocomplete-pelanggan li {
    padding: 5px 10px 5px 10px; 
    background:#FAFAFA; 
    border-bottom:#ddd 1px solid;
  }

  #daftar-autocomplete-pelanggan li:hover,
  #daftar-autocomplete-pelanggan li.autocomplete_active { 
    background:#6bb9f0; 
    color:#fff; 
    cursor: pointer;
  } 
...
<?php echo form_open(base_url().'pelunasanjual/proses_tambah'); ?>
				...
				<div class="panel-body title-pos body-blue">
					...
					<div class="form-group col-md-3">
					    	<label>ID Pelanggan</label>
					      	...
					      	<input type="text" readonly="true" name="idpelanggan" id="idpelanggan" required="" class="form-control" placeholder="ID">
					      	<div id="hasilpencarianpelanggan"></div>
					</div>	
					<div class="form-group col-md-4">
					    	<label>Nama Pelanggan</label> 
					    	<input type="text" required="" id="pelanggan" class="form-control" placeholder="Nama Pelanggan">
					</div>
					<div class="col-md-5">
		...
		<div class="panel panel-default">
		    	<div class="panel-body">
					    ...
					    <div class=" form-group col-md-8">
					  		<label>Alamat</label>
					  		<textarea class="form-control" placeholder="Alamat"  id="alamatpelanggan" style="height: 105px"></textarea>
					  	</div>
...
<script type="text/javascript">
	$(document).on('keyup', '#pelanggan', function(e){
		$("#idpelanggan").val('');
		$("#alamatpelanggan").val('');
		var keyword = $("#pelanggan").val();
		if(keyword !== ""){
			var charcode = e.which || e.keyCode;
			$.ajax({
				url: "<?php echo base_url(); ?>pelanggan/cari_pelanggan",
				type: "post",
				cache: false,
				data: "like=" + keyword,
				dataType: "json",
				success: function(json){
					$("#hasilpencarianpelanggan").show();
					$("#hasilpencarianpelanggan").html(json.datanya);
				}
			});       
		}else{
			$("#hasilpencarianpelanggan").hide();      
		}
	});

	$(document).on('keyup', '#idpelanggan', function(e){
		$("#idpelanggan").val('');
		$("#alamatpelanggan").val('');
		var keyword = $("#idpelanggan").val();
		if(keyword !== ""){
			var charcode = e.which || e.keyCode;
			$.ajax({
				url: "<?php echo base_url(); ?>pelanggan/cari_pelanggan",
	...
	$(document).on("click", "#daftar-autocomplete-pelanggan li", function(){
	...
	function get_penjualan(pelanggan){
		$.ajax({
			type: "post",
			data: "pelanggan=" + pelanggan,
			url: '<?php echo base_url(); ?>pelunasanjual/cari_faktur',
